...
def parse_connection_string(conn_str):
    ...
    if not conn_str or conn_str.startswith('UseDevelopmentStorage=true'):
        return None
...
# Parse connection and check availability - make it more robust
try:
    ...
    logging.warning(f"Storage initialization failed: {e}")
    storage_config = None
...
def create_auth_header(account_name, account_key, method, url_path, headers):
    ...
    try:
        ...
        logging.error(f"Failed to create auth header: {e}")
        return None
...
def make_storage_request(method, url_path, data=None):
    ...
    try:
        ...
        auth_header = create_auth_header(account_name, account_key, method, url_path, headers)
        if not auth_header:
            return None
