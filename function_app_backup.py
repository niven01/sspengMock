...
def main(req: func.HttpRequest) -> func.HttpResponse:
    """
    Main entry point for Azure Function - handles MockApiFunction endpoint
    """
    logging.info(f"main: Received {req.method} request to {req.url}")
    
    # This function handles the MockApiFunction endpoint directly
    # All requests to /api/MockApiFunction come here
    return mock_api_function_handler(req)
...
def mock_api_function_handler(req: func.HttpRequest) -> func.HttpResponse:
    """Handles all requests to the MockApiFunction endpoint"""
    logging.info("=" * 60)
    logging.info("MockApiFunction: CALLED!")
    logging.info("MockApiFunction: Method=%s URL=%s", req.method, req.url)
    logging.info("MockApiFunction: Headers=%s", dict(req.headers))
    logging.info("MockApiFunction: Query=%s", dict(req.params))
    logging.info("=" * 60)
    ...
    try:
        ...
        try:
            ...
            logging.info("MockApiFunction: parsed JSON body")
        except ValueError:
            try:
                body_text = req.get_body().decode("utf-8")
                record["body"] = body_text if body_text else None
                logging.info("MockApiFunction: parsed text body")
            except Exception:
                logging.warning("MockApiFunction: could not read body")
                record["body"] = None
        ...
        logging.info("MockApiFunction: stored request (total=%d)", len(REQUEST_LOG[path]))
        logging.info("MockApiFunction: STORED RECORD: %s", json.dumps(record, default=str))
        ...
        response_body = {
            ...
            "inspect_url": f"{req.url.split('/api/')[0]}/api/MockApiFunction"
        }
        ...
        logging.exception("MockApiFunction: FAILED with exception: %s", e)
        return func.HttpResponse(
